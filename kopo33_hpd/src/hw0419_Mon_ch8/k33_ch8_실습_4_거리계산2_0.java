...
public class k33_ch8_실습_4_거리계산2_0 {
	/*
	...
	public static void main(String[] args) throws IOException {			//메인함수, error 예외 처리 해줌
		...
		while ((k33_readtxt = k33_br.readLine()) != null) {		//반복문 : readtxt를 줄 별로 읽어서 null이 아니면 반복 (field_name으로 읽은 것 다음 줄부터 read)
			...
			String[] k33_field = k33_readtxt.split("\t");		//문자열 배열 field는 readtxt를 \t으로 나눈 것들을 요소로 받아서 정의함
																//txt파일은 \t으로 각 요소들이 구분되어짐
			if (k33_field[21] == "" || k33_field[20] == "" || k33_field[21].equals("0") || k33_field[20].equals("0")) {	
																					//field 배열의 21번째 요소와 20번째 요소가 공백이거나 0이면 
																					//4 조건 중 하나라도 만족하면 아무것도 하지 않음
			} else {												//그렇지 않으면 (field 배열의 21번째 요소와 20번째 요소가 둘 다 공백이 아니고 0이 아니면)
				System.out.printf("**[%d번째 항목]*****************\n", k33_LineCnt);	//줄 수 출력
				System.out.printf(" %s : %s\n", k33_field_name[6], k33_field[6]);		//field_name배열의 6번째 요소와 field배열의 6번째 요소 출력 (주차장지번주소 출력)
				System.out.printf(" %s : %s\n", k33_field_name[21], k33_field[21]);		//field_name배열의 21번째 요소와 field배열의 21번째 요소 출력 (위도 출력)
				System.out.printf(" %s : %s\n", k33_field_name[20], k33_field[20]);		//field_name배열의 20번째 요소와 field배열의 20번째 요소 출력 (경도 출력)
				
				double k33_dist = Math.sqrt( Math.pow(Double.parseDouble(k33_field[21])-k33_lat, 2)	//Math.sqrt와 Math.pow 이용하여 k33_집과 각 도로명주소의 위도 경도 비교하여
						+ Math.pow(Double.parseDouble(k33_field[20])-k33_lng, 2));					//k33_집과의 거리를 구한다음 구한 값을 실수형 변수 dist로 정의
				System.out.printf(" 현재지점과 거리 : %f\n",k33_dist);								//k33_집과의 거리 출력
				System.out.printf("********************************\n");							//문자열 출력

				if (k33_min > k33_dist) {						//조건문 : min이 dist보다 크면
					k33_min = k33_dist;							//min을 dist로 재정의하고
					k33_min_field = k33_field[6];				//min_field를 field 배열의 1번째 요소로 재정의
				} else if (k33_max < k33_dist) {				//조건문 : max가 dist보다 작으면
					k33_max = k33_dist;							//max를 dist로 재정의하고
					k33_max_field = k33_field[6];				//max_field를 field 배열의 1번째 요소로 재정의
				}
				k33_LineCnt++;									//줄 수 1 더해주기
			}
		}
		System.out.printf("%s : %s\n%s : %f\n", "최소거리지점", k33_min_field, "거리", k33_min);	//최종 최소거리지점 도로명주소와 k33_집과의 거리 출력
		System.out.printf("%s : %s\n%s : %f\n", "최대거리지점", k33_max_field, "거리", k33_max);	//최종 최대거리지점 도로명주소와 k33_집과의 거리 출력
